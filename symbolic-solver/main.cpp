...
void getMonomialOPB() {
  ...
  assert(s.size() >= 2);
  //coef = stringToInt(s.substr(1, s.length()-1));
...
inline void addConstraintToDB(vector<int>& coefficients, vector<int>& literals, int rhs, bool isGeq) {
  ...
  if (rhs >= 1) {
    constraints.push_back(WConstraint(coefficients,literals,rhs,{rhs,1},{0,1}, false));
  }
...
void showUsage(char* exec) {
  cout << exec << " [-help] [-id int] [-seed int] [-tlimit int] [-wperc double] [-card bool] [-prior bool] [-bt0 bool] [-symb bool] [-crhs bool] [-prhs bool][-multiobj bool] [-w executionInfo.txt] [-r executionInfo.txt] [-d decisionsLimit] [-c conflictsLimit] [-strategy file.txt] [-decision file.txt] [-iniSol file.txt]  *.lp/opb" << endl << endl;
  
  cout << "help     --> print this usage message" << endl;
  ...
  cout << "w        --> specify an output file to be writen, and solver set writeInfo to true." << endl;
  ...
  cout << "symb     --> (bool)    use symbolic conflict analyse or not (default is true)" << endl;
  cout << "crhs     --> (bool)    update the RHS of cardinality constraints based on symbolic inforamtion or not (default is true)" << endl;
  cout << "prhs     --> (bool)    update the RHS of PB constraints based on symbolic inforamtion or not (default is true)" << endl;
}
...
int main (int argc, char *argv[]) {  
  ...
  arg = inputReadCommandLineArg(argc,argv,"wperc");  // use watched propagation if the percentage of smallest number of watched literal is smaller than this percentage
  ...
  bool update_rhs = true;
  arg = inputReadCommandLineArg(argc,argv,"symb");   // use symbolic conflict analyse 
  if (arg) update_rhs = (string(arg) == "false"? false: (atoi(arg)==0)? false: true);
  
  bool card_rhs = true;
  arg = inputReadCommandLineArg(argc,argv,"crhs");  // update the rhs of symbolic cardinalitiy constraints
  if (arg) card_rhs = (string(arg) == "false"? false: (atoi(arg)==0)? false: true);
  
  bool pb_rhs = true;
  arg = inputReadCommandLineArg(argc,argv,"prhs");   // update the rhs of symbolic PB constraints
  if (arg) pb_rhs = (string(arg) == "false"? false: (atoi(arg)==0)? false: true);
  
  bool bt0 = true;
  arg = inputReadCommandLineArg(argc,argv,"bt0");    // backjump to dl 0 if a new solution is found
  if (arg) bt0 = (string(arg) == "false"? false: (atoi(arg)==0)? false: true);
  
  // not construct a new objective constraint after finding a new solution in order to directly update the rhs.
  bool multiObj = false;   
  ...
  cout << objCoeffs.size() << "  " << objVarNames.size() << " " << varNames.size() << "  " << coeffs.size() << "  " << varNums.size() << "  " << variables.size() << endl; 
  ...
  solver.setUpdateRHS(update_rhs);
  solver.setUpdatePBRHS(pb_rhs);
  solver.setUpdateCardRHS(card_rhs);
  solver.setBT0(bt0);
  solver.setMultipleObj(multiObj);
  solver.setPropagatebyPriority(propagate_by_priority);
  solver.setDecisionLimit(nDecs);
  solver.setConflictLimit(nConfs);
  ...
  for (int i = 0; i < (int)constraints.size(); i++) {
    ...
    assert(not constraints[i].isShaved());
  ...
  if(readInfoFile != "") {
    ifstream in(readInfoFile);
    if(not in.good()) {cout << "readInfoFile " << readInfoFile << " doesn't exist!" << endl; exit(0);}
    //string aux;
    //while(in >> aux) {
      //if (aux == "bestCost") {in >> aux; solver.bestCost = stoi(aux); break;}
      //solver.opt_solu.push_back(stoi(aux));
    //}
    //assert((int)solver.opt_solu.size() == numVars+1);
    //cout << "read bestCost " << solver.bestCost << endl;
  }
